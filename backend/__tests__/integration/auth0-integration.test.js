...
describe('Auth0Strategy Integration Tests', () => {
  ...
  describe('Authentication Flow', () => {
    ...
    test('should authenticate with valid authorization code', async () => {
      ...
      expect(mockAuthClient.users.getInfo).toHaveBeenCalledWith('mock-access-token');
    ...
    test('should handle user info retrieval failure', async () => {
      mockAuthClient.users.getInfo.mockRejectedValue(new Error('Failed to get user info'));
    ...
    test('should handle local user creation failure', async () => {
      UserDataService.findOrCreateUser.mockRejectedValue(new Error('Database error'));
  ...
  describe('Token Verification', () => {
    ...
    test('should reject expired token', async () => {
      ...
      await expect(
        auth0Strategy.verifyToken(validToken)
  ...
  describe('Error Handling & Edge Cases', () => {
    ...
    test('should reject invalid email format from Auth0', async () => {
      ...
      axios.post.mockResolvedValue({
        data: { access_token: 'token', id_token: 'id', refresh_token: 'refresh' },
      });
      auth0Strategy.verifyToken = jest.fn().mockResolvedValue({ sub: 'auth0|12345' });
