...
describe("Timeout Architecture", () => {
  ...
  beforeAll(async () => {
    ...
    const authResponse = await request(app).get("/api/dev/token?role=admin");
  ...
  describe("Request Timeout Middleware", () => {
    ...
    test("should add request timing metadata", async () => {
    ...
    test("should handle concurrent requests without timeout issues", async () => {
  ...
  describe("Database Query Timeouts", () => {
    test("should have query timeouts configured", async () => {
    ...
    test("should prevent hanging on database connection failure", async () => {
  ...
  describe("Timeout Hierarchy Validation", () => {
    test("should have proper timeout hierarchy", () => {
    ...
    test("should have health check timeout less than request timeout", () => {
    ...
    test("should have monitoring thresholds configured", () => {
      ...
      // Slow < Very Slow
      expect(TIMEOUTS.MONITORING.SLOW_REQUEST_MS).toBeLessThan(
        TIMEOUTS.MONITORING.VERY_SLOW_REQUEST_MS,
      );
    });
  });

  describe("Timeout Error Responses", () => {
    test("should return proper error format on timeout", async () => {
    ...
    test("should include timeout metadata in error response", () => {
  ...
  describe("Production Timeout Configuration", () => {
    test("should have reasonable production timeouts", () => {
    ...
    test("should have faster test timeouts", () => {
      // Test environment should have faster timeouts
      expect(TIMEOUTS.DATABASE.TEST.STATEMENT_TIMEOUT_MS).toBeLessThan(
        TIMEOUTS.DATABASE.STATEMENT_TIMEOUT_MS,
      );

      expect(TIMEOUTS.DATABASE.TEST.QUERY_TIMEOUT_MS).toBeLessThan(
        TIMEOUTS.DATABASE.QUERY_TIMEOUT_MS,
      );
    });
  });

  describe("Graceful Timeout Handling", () => {
    test("should not leak resources on timeout", async () => {
    ...
    test("should cleanup on request completion", async () => {
