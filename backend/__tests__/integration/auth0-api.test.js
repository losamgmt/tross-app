...
describe("Auth0 API Endpoints - Integration Tests", () => {
  ...
  describe("POST /api/auth0/callback - Exchange Auth Code", () => {
    test("should return 400 when code is missing", async () => {
      const response = await request(app).post("/api/auth0/callback").send({});
  ...
  describe("POST /api/auth0/validate - Validate ID Token (PKCE)", () => {
    test("should return 400 when id_token is missing", async () => {
      const response = await request(app).post("/api/auth0/validate").send({});
    ...
    test("should return 401 for expired token", async () => {
      ...
      const expiredToken =
        "eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJodHRwczovL2V4YW1wbGUuYXV0aDAuY29tLyIsInN1YiI6ImF1dGgwfDEyMzQ1Njc4OTAiLCJhdWQiOiJ0ZXN0LWF1ZGllbmNlIiwiZXhwIjoxNjAwMDAwMDAwLCJpYXQiOjE2MDAwMDAwMDB9.invalid_signature";
  ...
  describe("POST /api/auth0/refresh - Refresh Access Token", () => {
    test("should return 400 when refresh_token is missing", async () => {
      const response = await request(app).post("/api/auth0/refresh").send({});
  ...
  describe("GET /api/auth0/logout - Get Logout URL", () => {
    ...
    test("should return logout URL", async () => {
      const response = await request(app).get("/api/auth0/logout");
  ...
  describe("Rate Limiting", () => {
    test("should apply rate limiting to refresh endpoint", async () => {
      ...
      expect([HTTP_STATUS.UNAUTHORIZED, HTTP_STATUS.BAD_REQUEST]).toContain(
        response.status,
      );
    });
  });
