...
function hasCustomerOwnership(meta) {
  ...
  return Object.keys(foreignKeys).some(fk => 
    fk === 'customer_id' || 
    fk.endsWith('_customer_id')
  );
}
...
function hasTechnicianAssignment(meta) {
  ...
  return Object.keys(foreignKeys).some(fk => 
    fk === 'assigned_technician_id' || 
    fk.includes('technician')
  );
}
...
function customerSeesOnlyOwnWorkOrders(meta, ctx) {
  ...
  ctx.it(`GET /api/${meta.tableName} - customer sees only their own work orders`, async () => {
    // Get customer user
...
function technicianSeesOnlyAssignedWorkOrders(meta, ctx) {
  ...
  ctx.it(`GET /api/${meta.tableName} - technician sees only assigned work orders`, async () => {
    // Get technician user
...
function customerSeesOnlyOwnInvoices(meta, ctx) {
  ...
  ctx.it(`GET /api/${meta.tableName} - customer sees only their own invoices`, async () => {
