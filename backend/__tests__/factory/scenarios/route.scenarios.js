...
function requiresMinimumRole(routeMeta, endpointMeta, ctx) {
  ...
  ctx.it(
    `returns 403 for ${insufficientRole} role (requires ${minRole})`,
    async () => {
...
function handlesNotFound(routeMeta, endpointMeta, ctx) {
  ...
  const fullPath = buildPath(
    routeMeta.basePath,
    endpointMeta.path.replace(":id", "999999"),
  );
...
function actionHandlesInvalidInput(routeMeta, endpointMeta, ctx) {
  ...
  const fullPath = buildPath(routeMeta.basePath, endpointMeta.path).replace(
    "/1/",
    "/999999/",
  ); // Replace test ID with non-existent
...
function errorsDoNotLeakData(routeMeta, endpointMeta, ctx) {
  ...
  const badPath = buildPath(
    routeMeta.basePath,
    endpointMeta.path.replace(/:id\b/g, "invalid"),
  );
    ...
    if (response.status >= 400) {
      ...
      if (response.body.message) {
        ...
        ctx
          ...
          .not.toMatch(/\bSELECT\b|\bINSERT\b|\bUPDATE\b|\bDELETE\b|\bFROM\b/i);
      }
    }
  });
}
