...
function readByIdNotFound(meta, ctx) {
  ctx.it(`GET /api/${meta.tableName}/:id - returns 404 for non-existent`, async () => {
...
function updateMutableFields(meta, ctx) {
  ...
  ctx.it(`PATCH /api/${meta.tableName}/:id - updates ${fieldToUpdate}`, async () => {
    const created = await ctx.factory.create(meta.entityName);
...
function updateFailsOnImmutableFields(meta, ctx) {
  ...
  for (const field of immutableFields) {
    ...
    ctx.it(`PATCH /api/${meta.tableName}/:id - rejects update to ${field}`, async () => {
      const created = await ctx.factory.create(meta.entityName);
...
function deleteWithDependents(meta, ctx) {
  ...
  ctx.it(`DELETE /api/${meta.tableName}/:id - cascades to dependents`, async () => {
    const { entity, dependentRecords } = await ctx.factory.createWithDependents(
      meta.entityName
    );
    const auth = await ctx.authHeader('admin');
