...
const testLog = (...args) =>
  originalLog("[HEALTH-TEST]", new Date().toISOString(), ...args);
...
describe("Health Routes", () => {
  ...
  beforeEach(() => {
    ...
    requireMinimumRole.mockImplementation((role) => {
      testLog(
        `AUTH: requireMinimumRole(${role}) called - returning middleware`,
      );
      return (req, res, next) => {
        testLog(
          `AUTH: requireMinimumRole middleware executing - passing through`,
        );
        next();
      };
    });
  ...
  describe("GET /api/health/databases", () => {
    ...
    test("should return degraded status for slow DB (100-500ms)", async () => {
      ...
      try {
        ...
        // Assert
        expect(response.body.data.databases[0].status).toBe("degraded");
        expect(response.body.data.databases[0].responseTime).toBeGreaterThan(
          100,
        );
