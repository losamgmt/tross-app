  ...
  describe("exportToCSV()", () => {
    it("should export all non-sensitive fields by default", async () => {
      ...
      expect(result.filename).toMatch(
        /^customer_export_\d{4}-\d{2}-\d{2}\.csv$/,
      );

      // Check CSV content
      expect(result.csv).toContain(
        "ID,First Name,Last Name,Email,Phone,Active,Created At",
      );
    ...
    it("should export only selected fields when specified", async () => {
      db.query.mockResolvedValueOnce({
        rows: [{ first_name: "John", email: "john@example.com" }],
      });

      const result = await ExportService.exportToCSV("customer", {}, null, [
        "first_name",
        "email",
      ]);
    ...
    it("should throw error for unknown entity", async () => {
      await expect(ExportService.exportToCSV("unknown_entity")).rejects.toThrow(
