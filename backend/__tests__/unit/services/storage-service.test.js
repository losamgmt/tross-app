...
describe('StorageService', () => {
  ...
  describe('generateStorageKey()', () => {
    ...
    test('handles complex filenames with multiple dots', () => {
      const key = service.generateStorageKey('work_order', 2, 'report.2024.final.pdf');
  ...
  describe('getSignedDownloadUrl()', () => {
    test('returns signed URL from presigner', async () => {
      mockGetSignedUrl.mockResolvedValueOnce('https://signed-url.example.com/file?token=xyz');

      const url = await service.getSignedDownloadUrl('work_order/123/photo.jpg');
    ...
    test('propagates presigner errors', async () => {
      ...
      await expect(
        service.getSignedDownloadUrl('test/key'),
  ...
  describe('getMetadata()', () => {
    ...
    test('propagates errors when file not found', async () => {
      ...
      await expect(
        service.getMetadata('missing/key'),
