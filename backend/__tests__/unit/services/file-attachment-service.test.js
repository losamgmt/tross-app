...
describe("FileAttachmentService", () => {
  ...
  describe("entityExists()", () => {
    ...
    test("should return false when entity key not in metadata", async () => {
      const result = await FileAttachmentService.entityExists(
        "nonexistent_entity",
        1,
      );
      ...
      expect(logger.warn).toHaveBeenCalledWith(
        "Unknown entity key: nonexistent_entity",
      );
    });
  ...
  describe("listFilesForEntity()", () => {
    test("should return files for entity", async () => {
      ...
      const result = await FileAttachmentService.listFilesForEntity(
        "work_order",
        5,
      );
  ...
  describe("softDelete()", () => {
    test("should soft delete and return true", async () => {
      ...
      expect(logger.info).toHaveBeenCalledWith("File attachment soft-deleted", {
        id: 1,
      });
    });
  ...
  describe("formatForResponse()", () => {
    ...
    test("should exclude storage_key from response", async () => {
      ...
      const result = FileAttachmentService.formatForResponse(
        row,
        mockDownloadInfo,
      );
    ...
    test("should throw if downloadInfo is missing", () => {
      ...
      expect(() =>
        FileAttachmentService.formatForResponse(row, { url: "x" }),
      ).toThrow("downloadInfo with url and expiresAt is required");
    });
  });
});
