  ...
  describe("sanitizeIdentifier", () => {
    ...
    describe("invalid identifiers", () => {
      ...
      it("should reject SQL injection attempts", () => {
        ...
        expect(() => sanitizeIdentifier("users'; DELETE FROM users--")).toThrow(
          "contains invalid characters",
        );
      ...
      it("should reject identifiers with spaces", () => {
        expect(() => sanitizeIdentifier("user table")).toThrow(
          "contains invalid characters",
        );
      });
      ...
      it("should reject identifiers starting with number", () => {
        expect(() => sanitizeIdentifier("123users")).toThrow(
          "contains invalid characters",
        );
      });
    });
    ...
    describe("custom context", () => {
      it("should include custom context in error message", () => {
        ...
        expect(() => sanitizeIdentifier(123, "column")).toThrow(
          "Invalid column",
        );
      });
    });
  ...
  describe("validateFieldAgainstWhitelist", () => {
    ...
    describe("valid fields", () => {
      it("should return field when in whitelist", () => {
        expect(validateFieldAgainstWhitelist("name", allowedFields)).toBe(
          "name",
        );
        expect(validateFieldAgainstWhitelist("email", allowedFields)).toBe(
          "email",
        );
      });
    ...
    describe("invalid fields", () => {
      it("should throw when field not in whitelist", () => {
        expect(() =>
          validateFieldAgainstWhitelist("unknown", allowedFields),
        ).toThrow('Invalid field: "unknown" is not allowed');
      });

      it("should include allowed values in error message", () => {
        expect(() =>
          validateFieldAgainstWhitelist("invalid", allowedFields),
        ).toThrow("Allowed values: id, name, email, status");
      });

      it("should be case sensitive", () => {
        expect(() =>
          validateFieldAgainstWhitelist("Name", allowedFields),
    ...
    describe("invalid whitelist", () => {
      it("should throw when whitelist is not an array", () => {
        expect(() => validateFieldAgainstWhitelist("field", null)).toThrow(
          "whitelist must be an array",
        );
        expect(() =>
          validateFieldAgainstWhitelist("field", "not-array"),
        ).toThrow("whitelist must be an array");
        expect(() => validateFieldAgainstWhitelist("field", {})).toThrow(
          "whitelist must be an array",
        );
      });
    ...
    describe("custom context", () => {
      it("should include custom context in error message", () => {
        expect(() =>
          validateFieldAgainstWhitelist("unknown", allowedFields, "sort field"),
