...
describe('Name Utils', () => {
  ...
  describe('truncate()', () => {
    ...
    test('uses default max length of 30', () => {
      const longText = 'This is a very long description that exceeds 30 characters';
  ...
  describe('computeName()', () => {
    ...
    test('truncates long summary to 50 characters', () => {
      const longSummary = 'This is a very long summary that should be truncated to 50 characters for display';
      const result = computeName({
        entity: { summary: longSummary, work_order_number: 'WO-001' },
        customer: { first_name: 'Jane' },
        identifierField: 'work_order_number',
      });
      expect(result).toContain('This is a very long summary that should be truncat...');
    });
  });
