...
describe("utils/validation-loader.js", () => {
  ...
  describe("FIELD definitions integration", () => {
    ...
    const fieldsWithMaxLength = fieldEntries.filter(
      ([_, def]) => def.maxLength !== undefined,
    );

    describe.each(fieldsWithMaxLength)(
      "FIELD.%s maxLength enforcement",
      (fieldName, fieldDef) => {
