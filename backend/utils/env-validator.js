...
const REQUIRED_ENV_VARS = {
  ...
  DB_PORT: {
    ...
    default: "5432",
    validator: (val) =>
      !isNaN(val) && parseInt(val) > 0 && parseInt(val) < 65536,
  ...
  JWT_SECRET: {
    ...
    validator: (val) =>
      val.length >= 16 &&
      !["your-secret-key", "change-me", "dev-secret-key"].includes(
        val.toLowerCase(),
      ),
    error:
      "JWT_SECRET must be at least 16 characters and not a default value (in production, use 64+ chars with mixed case, numbers, special chars)",
    sensitive: true,
  },
...
const PRODUCTION_CHECKS = {
  JWT_SECRET: {
    ...
    error:
      "JWT_SECRET in production must be 64+ characters with uppercase, lowercase, numbers, and special characters",
  },
