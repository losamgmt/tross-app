...
async function resetDatabase(config, envName) {
  ...
  try {
    ...
    rolesResult.rows.forEach((r) => {
      ...
      log(`   ${emoji} ID: ${r.id}, Name: ${r.name.padEnd(12)}, Priority: ${r.priority}`, 'cyan');
    });
    ...
    log(`\n👥 Users in ${envName}: ${userCount}`, userCount === 0 ? 'green' : 'yellow');
    ...
    if (envName === 'DEVELOPMENT') {
      ...
      if (adminCheck.rows.length > 0) {
        log(`   ✅ Admin user exists: ${adminCheck.rows[0].email} (role_id: ${adminCheck.rows[0].role_id})`, 'green');
      } else {
        log('   ℹ️  No admin user yet (will be created on first Auth0 login)', 'blue');
      }
    }
